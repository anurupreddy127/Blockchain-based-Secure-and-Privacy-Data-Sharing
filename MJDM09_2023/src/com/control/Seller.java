...
public class Seller extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String sql = "select * from seller where email='"+uid+"' and password='"+pwd+"'";
		HttpSession session = request.getSession();
		if(Dao.login(sql)){
			session.setAttribute("uid", uid);
			sql = "select name from seller where email='"+uid+"'";
			String name = Dao.getName(sql);
			session.setAttribute("name", name);
			sql = "select company from seller where email='"+uid+"'";
			...
			o.println("window.location='seller.jsp';</script>");
		}
	}
